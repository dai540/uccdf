...
[![License:
MIT](https://img.shields.io/badge/license-MIT-green.svg)](https://dai540.github.io/uccdf/LICENSE)

`uccdf` is an R package for typed consensus clustering on structured
...
The quickest way to understand a fit is to inspect these three views
together:

- the `K` selection table from
  [`select_k()`](https://dai540.github.io/uccdf/reference/select_k.md)
- the cluster-colored latent scatter from
  [`plot_embedding()`](https://dai540.github.io/uccdf/reference/plot_embedding.md)
- the hierarchical consensus heatmap from
...
This fixes the earlier failure mode where examples could collapse to
`K = 1` simply because `n_null` was too small to support meaningful
...
- `exploratory_cluster` exposes the strongest unsupported multi-cluster
  split

That keeps the reported result honest without hiding structure from the
analyst.
...
- [`validate_input()`](https://dai540.github.io/uccdf/reference/validate_input.md)
  checks identifier integrity and structural assumptions
- [`infer_schema()`](https://dai540.github.io/uccdf/reference/infer_schema.md)
  marks columns as `id`, `active`, or `exclude`
...
- [`fit_uccdf()`](https://dai540.github.io/uccdf/reference/fit_uccdf.md)
  runs multiple clustering jobs and builds consensus matrices
- [`select_k()`](https://dai540.github.io/uccdf/reference/select_k.md)
  returns the stability and null-calibrated ranking table
- [`augment()`](https://dai540.github.io/uccdf/reference/augment.md)
  returns row-level selected and exploratory assignments
- [`report()`](https://dai540.github.io/uccdf/reference/report.md)
  writes a compact markdown or html summary
- [`plot_embedding()`](https://dai540.github.io/uccdf/reference/plot_embedding.md)
  shows low-dimensional separation
- [`plot_consensus_heatmap()`](https://dai540.github.io/uccdf/reference/plot_consensus_heatmap.md)
  shows hierarchical agreement structure
...
`confidence` is interpreted as a consensus-derived assignment stability
score, not as a Bayesian posterior probability.

## Example data

`toy_mixed_data` is bundled for examples, tests, and vignettes. It
includes:
...
- `Articles`
  ...
  - real dataset analyses with dataset-specific background, objective,
...
- It does not yet ship advanced null generators such as copula-based
  nulls.
- It does not yet include `k-prototypes` or `KAMILA` in the default
  workflow.

The goal of the first release is a stable and readable foundation for
typed consensus clustering on structured tables.
